...
public class RadiosityShader extends GVRShader {
    ...
    private static final String FRAGMENT_SHADER = "" //
            ...
            + "  if( u_weight.x < 0.999 ) { color1 = bg*(u_lightness.x*(1.0-0.4*depth)*v_screen_color); }\n"
            + "  if( u_weight.x > 0.001 ) { color2 = bg*0.15; }\n"
            + "  float alpha = min( 1.0, 2.0 - u_weight.x );\n"
            + "  FragColor = vec4( u_fade.x*(color1*(1.0-u_weight.x)+color2*u_weight.x), alpha);\n"
            + "}\n";
    ...
    public RadiosityShader(GVRContext gvrContext) {
        ...
        super("float u_weight, float u_fade, float u_lightness", "samplerExternalOES u_screen sampler2D u_texture_off", "float4 a_position, float3 a_normal, float2 a_tex_coord", 300);
